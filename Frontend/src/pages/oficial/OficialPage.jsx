...
function OficialPage({ onCreate }) {
  ...
  const provincias = useMemo(() => {
    return uniqueValues(
      actas.filter(
        (item) =>
          item.departamento === departamento && item.municipio === municipio
      ),
      "provincia"
    );
  }, [actas, departamento, municipio]);
  ...
  }, [actas, departamento, municipio, provincia]);
  ...
  return (
    <div className="ofi-root">
      <header className="ofi-header">
        <div className="ofi-header-content">
          <div>
            <h2 className="page-title">Archivo Oficial de Actas</h2>
            <p className="page-description">
              Buscador y visor detallado de mesas procesadas e integradas al
              escrutinio final.
            </p>
          </div>
          ...
          {onCreate && (
            <button
              ...
              onClick={onCreate}
              title="Registrar transcripción oficial"
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
              ...
              <span>Registrar transcripción</span>
            </button>
          )}
        </div>
      </header>
        ...
        <div className="ofi-selected-recinto animate-fade-in">
          <div className="recinto-info">
            ...
            <div>
              ...
              <p>
                Has seleccionado un recinto con <b>{mesasFiltradas.length}</b>{" "}
                mesa(s) oficial(es) registrada(s).
              </p>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {mesasFiltradas.map((acta) => (
                <tr key={acta.codigo_acta}>
                  ...
                  <td>
                    <strong>{acta.nro_mesa}</strong>
                  </td>
                  <td className="ofi-truncate" title={acta.nombre_recinto}>
                    {acta.nombre_recinto}
                  </td>
                  <td className="text-center">
                    <span className="ofi-badge badge-success">
                      {acta.votos_validos}
                    </span>
                  </td>
                  <td className="text-center">
                    <span className="ofi-badge badge-neutral">
                      {acta.votos_blancos}
                    </span>
                  </td>
                  <td className="text-center">
                    <span className="ofi-badge badge-danger">
                      {acta.votos_nulos}
                    </span>
                  </td>
                  <td className="text-center">
              ...
              {mesasFiltradas.length === 0 && (
                <tr>
                  <td colSpan="11" className="ofi-empty-row">
                    ...
                    Utiliza los filtros superiores para cargar las actas de un
                    recinto específico.
                  </td>
                </tr>
              )}
          ...
          <div className="ofi-modal">
            ...
            <div className="ofi-modal-grid">
              <section className="modal-section">
                ...
                <div className="modal-data-list">
                  <div className="data-row">
                    <span>Departamento:</span> <b>{selectedActa.departamento}</b>
                  </div>
                  <div className="data-row">
                    <span>Municipio:</span> <b>{selectedActa.municipio}</b>
                  </div>
                  <div className="data-row">
                    <span>Provincia:</span> <b>{selectedActa.provincia}</b>
                  </div>
                  <div className="data-row highlight-row">
                    <span>Recinto:</span> <b>{selectedActa.nombre_recinto}</b>
                  </div>
                  <div className="data-row">
                    <span>Dirección:</span>{" "}
                    <b
                      className="truncate-text"
                      title={selectedActa.direccion_recinto}
                    >
                      {selectedActa.direccion_recinto}
                    </b>
                  </div>
                  <div className="data-row big-row">
                    <span>Nro. de Mesa:</span>{" "}
                    <strong>{selectedActa.nro_mesa}</strong>
                  </div>
                </div>
              </section>
              ...
              <section className="modal-section">
                ...
                <div className="modal-data-list">
                  <div className="data-row">
                    <span>Votantes Habilitados:</span>{" "}
                    <b>{selectedActa.votantes_habilitados}</b>
                  </div>
                  <div className="data-row">
                    <span>Papeletas en Ánfora:</span>{" "}
                    <b>{selectedActa.papeletas_anfora}</b>
                  </div>
                  <div className="data-row">
                    <span>Papeletas No Utilizadas:</span>{" "}
                    <b>{selectedActa.papeletas_no_utilizadas}</b>
                  </div>
                  ...
                  <div className="data-row text-success">
                    ...
                    <strong>{selectedActa.votos_validos}</strong>
                  </div>
                  <div className="data-row text-neutral">
                    <span>Votos Blancos:</span>{" "}
                    <strong>{selectedActa.votos_blancos}</strong>
                  </div>
                  <div className="data-row text-danger">
                    <span>Votos Nulos:</span>{" "}
                    <strong>{selectedActa.votos_nulos}</strong>
                  </div>
                </div>
              </section>
